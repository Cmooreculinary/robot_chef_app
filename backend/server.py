...
GLOBAL_MISSIONS = [{"id": "rice", "title": "Rice Around the World", "region": "Global"}]
FAMILY_CHALLENGES = [{"id": "fc-1", "title": "Three Safety Zones", "prompt": "Check the kitchen zones."}]
...
async def list_lessons(): return {"items": LESSONS}
...
async def get_progress(session_id: str):
    ...
    row = cursor.fetchone()
    conn.close()
    if not row: return ProgressDoc(sessionId=session_id).model_dump()
    doc = dict(row)
...
async def list_food_trucks(session_id: str): return {"items": []}

@api_router.post("/builders/restaurant")
async def save_restaurant(payload: RestaurantConcept): return payload

@api_router.get("/builders/restaurant/{session_id}")
async def list_restaurants(session_id: str): return {"items": []}
